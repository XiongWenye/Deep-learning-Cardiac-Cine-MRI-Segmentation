...
from torchvision import transforms # Kept for potential future use, but not active in this version
import os
import random
...
def convert_to_multi_labels(label_tensor):
    ...
    # Class 1 (e.g., RV - typically red, highest value)
    mask1 = (label_tensor >= 250).squeeze(1)
    # Class 2 (e.g., Myo - typically green, middle value)
    mask2 = ((label_tensor >= 165) & (label_tensor < 250-1)).squeeze(1)
    # Class 3 (e.g., LV - typically blue, lower non-zero value)
    mask3 = ((label_tensor >= 80) & (label_tensor < 165-1)).squeeze(1)
...
class Up(nn.Module): # Modified for Attention U-Net
    def __init__(self, in_channels_decoder, in_channels_encoder, out_channels, bilinear=True):
        ...
        if bilinear:
            ...
            self.up = nn.ConvTranspose2d(in_channels_decoder, in_channels_decoder // 2, kernel_size=2, stride=2)
        ...
        self.att = AttentionBlock(F_g=upsampled_decoder_channels, F_l=in_channels_encoder, F_int=in_channels_encoder // 2)
        ...
        self.conv = DoubleConv(upsampled_decoder_channels + in_channels_encoder, out_channels)

    def forward(self, x_decoder, x_encoder): # x_decoder from lower layer, x_encoder is skip connection
        ...
        x_att = self.att(g, x_encoder) # Apply attention to encoder features using g as context
...
class AttentionUNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True, C_base=32): # Using C_base=32 from your original baseline
...
def save_segmentation_results(model, dataset_to_sample_from, device, output_base_dir, num_samples=3, model_name=""):
    ...
    for i, sample_idx in enumerate(random_indices):
        ...
        if len(class_names) == 1: axes = axes.reshape(1, -1)
...
attention_unet_model = AttentionUNet(n_channels=1, n_classes=3, bilinear=True, C_base=32) 
optimizer_att_unet = torch.optim.Adam(attention_unet_model.parameters(), lr=0.001) # You might want to tune LR
lr_scheduler_att_unet = torch.optim.lr_scheduler.ExponentialLR(optimizer=optimizer_att_unet, gamma=0.95)
...
for images, labels_gt in dataloader_test:
    ...
    labels_multilabel = convert_to_multi_labels(labels_gt) # (B, 3, H, W) float
    ...
    dice_scores_att_unet.append((dice_rv.item(), dice_myo.item(), dice_lv.item()))

    # Accuracy Scores
    acc_rv = get_accuracy(preds_binary[:, 0, :, :], labels_multilabel[:, 0, :, :])
    acc_myo = get_accuracy(preds_binary[:, 1, :, :], labels_multilabel[:, 1, :, :])
    acc_lv = get_accuracy(preds_binary[:, 2, :, :], labels_multilabel[:, 2, :, :])
    accuracy_scores_att_unet.append((acc_rv, acc_myo, acc_lv)) # get_accuracy might return float directly

mean_dice_rv_att = np.mean([score[0] for score in dice_scores_att_unet])
std_dice_rv_att = np.std([score[0] for score in dice_scores_att_unet])
...
mean_dice_lv_att = np.mean([score[2] for score in dice_scores_att_unet])
std_dice_lv_att = np.std([score[2] for score in dice_scores_att_unet])

print(f'Attention U-Net - RV Dice Coefficient: Mean={mean_dice_rv_att:.4f}, SD={std_dice_rv_att:.4f}')
print(f'Attention U-Net - MYO Dice Coefficient: Mean={mean_dice_myo_att:.4f}, SD={std_dice_myo_att:.4f}')
print(f'Attention U-Net - LV Dice Coefficient: Mean={mean_dice_lv_att:.4f}, SD={std_dice_lv_att:.4f}')

mean_acc_rv_att = np.mean([score[0] for score in accuracy_scores_att_unet])
std_acc_rv_att = np.std([score[0] for score in accuracy_scores_att_unet])
...
mean_acc_lv_att = np.mean([score[2] for score in accuracy_scores_att_unet])
std_acc_lv_att = np.std([score[2] for score in accuracy_scores_att_unet])

print(f'Attention U-Net - RV Accuracy: Mean={mean_acc_rv_att:.4f}, SD={std_acc_rv_att:.4f}')
print(f'Attention U-Net - MYO Accuracy: Mean={mean_acc_myo_att:.4f}, SD={std_acc_myo_att:.4f}')
print(f'Attention U-Net - LV Accuracy: Mean={mean_acc_lv_att:.4f}, SD={std_acc_lv_att:.4f}')

# Save segmentation results for Attention U-Net
save_segmentation_results(attention_unet_model, test_set, device, 'result', num_samples=3, model_name='attention_unet')
...
with open(output_file_path, 'w') as file:
    ...
    file.write(f'RV Dice Coefficient: Mean={mean_dice_rv_att:.4f}, SD={std_dice_rv_att:.4f}\n')
    file.write(f'MYO Dice Coefficient: Mean={mean_dice_myo_att:.4f}, SD={std_dice_myo_att:.4f}\n')
    file.write(f'LV Dice Coefficient: Mean={mean_dice_lv_att:.4f}, SD={std_dice_lv_att:.4f}\n\n')
...
print("Segmentation sample images for Attention U-Net saved in 'result/attention_unet' subdirectory.")